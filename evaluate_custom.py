...
from utils.miou import compute_mean_ioU, write_results_png, compute_mean_ioU_4classes
...
def get_lip_palette():
    palette = [0,0,0,   #Background         0
            128,0,0,    #Hat                1
            255,0,0,    #Hair               2
            0,85,0,     #Glove              3
            170,0,51,   #Sunglasses         4
            255,85,0,   #Upper-clothes      5
            0,0,85,     #Dress              6
            0,119,221,  #Coat               7
            85,85,0,    #Socks              8
            0,85,85,    #Pants              9
            85,51,0,    #Jumpsuits          10
            52,86,128,  #Scarf              11
            0,128,0,    #Skirt              12
            0,0,255,    #Face               13
            51,170,221, #Left-arm           14
            0,255,255,  #Right-arm          15
            85,255,170, #Left-leg           16
            170,255,85, #Right-leg          17
            255,255,0,  #Left-shoe          18
            255,170,0]  #Right-shoe         19
    return palette
...
def valid(model, valloader, input_size, num_samples, gpus):
    ...
    with torch.no_grad():
        for index, batch in enumerate(valloader):
            ...
            # Make data_list for computing metrics
            for _ in range(num_images):
                temp = dict()
                temp["im_name"] = meta['name'][0]
                temp["img_width"] = int(meta['width'])
                temp["img_height"] = int(meta['height'])
                print(temp)
    ...
    return parsing_preds, scales, centers, data_list
...
def main():
    ...
    lip_dataset = LIPDataValSet(args.data_dir, 'val', crop_size=input_size, transform=transform)
    ...
    parsing_preds, scales, centers, data_list = valid(model, valloader, input_size, num_samples, len(gpus))
    ...
    #Save CDGNet results
    dir_gt = "gt가 저장된 경로"
    dir_pred = "model result 저장할 경로"
    write_results_png(data_list, parsing_preds, scales, centers, dir_pred, input_size)

    #Compute metrics
    mIoU = compute_mean_ioU_4classes(dir_pred, dir_gt, num_classes=4, dataset='LIP', dsize=(473,473))
    print(mIoU)
